...
create table if not exists wkc_job
(
    id int auto_increment
        primary key,
    type varchar(32) null comment '任务类型',
    url varchar(512) null comment '网址',
    create_time timestamp default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP,
    user_id bigint null comment '归属用户',
    wkc_user_id int null comment '归属玩客云用户',
    parent_id int null,
    exception_msg varchar(255) null,
    status int null comment '0 未处理   1 已处理   -1   处理失败'
);

create table if not exists wkc_user
(
    id int auto_increment
        primary key,
    phone varchar(32) null comment '手机号',
    user_id longtext null comment '关联用户',
    password varchar(64) null comment '密码',
    token varchar(128) null comment '玩客云token',
    account_type int null,
    enable_home_share int null,
    bind_pwd int null,
    phone_area int null,
    bind_user bigint null comment '归属用户',
    default_device_id varchar(255) null comment '默认节点',
    default_usb_uuid varchar(255) null comment '默认USB设备UUID',
    default_usb_path varchar(255) null comment '默认USB设备存储路径',
    default_peer_id varchar(255) null,
    uuid varchar(128) null comment 'uuid'
)
    comment '玩客云用户表';


create table if not exists eladmin.ACT_EVT_LOG
(
...
create table if not exists eladmin.ACT_GE_BYTEARRAY
(
...
create table if not exists eladmin.ACT_GE_PROPERTY
(
...
create table if not exists eladmin.ACT_HI_ACTINST
(
    ID_ varchar(64) not null
        primary key,
...
create index ACT_IDX_HI_ACT_INST_END
    on eladmin.ACT_HI_ACTINST (END_TIME_);

create index ACT_IDX_HI_ACT_INST_EXEC
    on eladmin.ACT_HI_ACTINST (EXECUTION_ID_, ACT_ID_);

create index ACT_IDX_HI_ACT_INST_PROCINST
    on eladmin.ACT_HI_ACTINST (PROC_INST_ID_, ACT_ID_);

create index ACT_IDX_HI_ACT_INST_START
    on eladmin.ACT_HI_ACTINST (START_TIME_);

create table if not exists eladmin.ACT_HI_ATTACHMENT
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create table if not exists eladmin.ACT_HI_COMMENT
(
    ID_ varchar(64) not null
        primary key,
    TYPE_ varchar(255) null,
...
create table if not exists eladmin.ACT_HI_DETAIL
(
    ID_ varchar(64) not null
        primary key,
...
create index ACT_IDX_HI_DETAIL_ACT_INST
    on eladmin.ACT_HI_DETAIL (ACT_INST_ID_);

create index ACT_IDX_HI_DETAIL_NAME
    on eladmin.ACT_HI_DETAIL (NAME_);

create index ACT_IDX_HI_DETAIL_PROC_INST
    on eladmin.ACT_HI_DETAIL (PROC_INST_ID_);

create index ACT_IDX_HI_DETAIL_TASK_ID
    on eladmin.ACT_HI_DETAIL (TASK_ID_);

create index ACT_IDX_HI_DETAIL_TIME
    on eladmin.ACT_HI_DETAIL (TIME_);

create table if not exists eladmin.ACT_HI_IDENTITYLINK
(
    ID_ varchar(64) not null
        primary key,
...
create index ACT_IDX_HI_IDENT_LNK_PROCINST
    on eladmin.ACT_HI_IDENTITYLINK (PROC_INST_ID_);

create index ACT_IDX_HI_IDENT_LNK_TASK
    on eladmin.ACT_HI_IDENTITYLINK (TASK_ID_);

create index ACT_IDX_HI_IDENT_LNK_USER
    on eladmin.ACT_HI_IDENTITYLINK (USER_ID_);

create table if not exists eladmin.ACT_HI_PROCINST
(
    ID_ varchar(64) not null
        primary key,
    PROC_INST_ID_ varchar(64) not null,
...
create index ACT_IDX_HI_PRO_INST_END
    on eladmin.ACT_HI_PROCINST (END_TIME_);

create index ACT_IDX_HI_PRO_I_BUSKEY
    on eladmin.ACT_HI_PROCINST (BUSINESS_KEY_);

create table if not exists eladmin.ACT_HI_TASKINST
(
    ID_ varchar(64) not null
        primary key,
    PROC_DEF_ID_ varchar(64) null,
...
create index ACT_IDX_HI_TASK_INST_PROCINST
    on eladmin.ACT_HI_TASKINST (PROC_INST_ID_);

create table if not exists eladmin.ACT_HI_VARINST
(
    ID_ varchar(64) not null
        primary key,
    PROC_INST_ID_ varchar(64) null,
...
create index ACT_IDX_HI_PROCVAR_NAME_TYPE
    on eladmin.ACT_HI_VARINST (NAME_, VAR_TYPE_);

create index ACT_IDX_HI_PROCVAR_PROC_INST
    on eladmin.ACT_HI_VARINST (PROC_INST_ID_);

create index ACT_IDX_HI_PROCVAR_TASK_ID
    on eladmin.ACT_HI_VARINST (TASK_ID_);

create table if not exists eladmin.ACT_PROCDEF_INFO
(
    ID_ varchar(64) not null
        primary key,
    PROC_DEF_ID_ varchar(64) not null,
...
create index ACT_IDX_INFO_PROCDEF
    on eladmin.ACT_PROCDEF_INFO (PROC_DEF_ID_);

create table if not exists eladmin.ACT_RE_DEPLOYMENT
(
    ID_ varchar(64) not null
        primary key,
    NAME_ varchar(255) null,
...
create table if not exists eladmin.ACT_RE_MODEL
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create table if not exists eladmin.ACT_RE_PROCDEF
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create table if not exists eladmin.ACT_RU_DEADLETTER_JOB
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create table if not exists eladmin.ACT_RU_EVENT_SUBSCR
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create index ACT_IDX_EVENT_SUBSCR_CONFIG_
    on eladmin.ACT_RU_EVENT_SUBSCR (CONFIGURATION_);

create table if not exists eladmin.ACT_RU_EXECUTION
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create index ACT_IDC_EXEC_ROOT
    on eladmin.ACT_RU_EXECUTION (ROOT_PROC_INST_ID_);

create index ACT_IDX_EXEC_BUSKEY
    on eladmin.ACT_RU_EXECUTION (BUSINESS_KEY_);

create table if not exists eladmin.ACT_RU_IDENTITYLINK
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create index ACT_IDX_ATHRZ_PROCEDEF
    on eladmin.ACT_RU_IDENTITYLINK (PROC_DEF_ID_);

create index ACT_IDX_IDENT_LNK_GROUP
    on eladmin.ACT_RU_IDENTITYLINK (GROUP_ID_);

create index ACT_IDX_IDENT_LNK_USER
    on eladmin.ACT_RU_IDENTITYLINK (USER_ID_);

create table if not exists eladmin.ACT_RU_INTEGRATION
(
    ID_ varchar(64) not null
        primary key,
    EXECUTION_ID_ varchar(64) null,
...
create table if not exists eladmin.ACT_RU_JOB
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create table if not exists eladmin.ACT_RU_SUSPENDED_JOB
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create table if not exists eladmin.ACT_RU_TASK
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create index ACT_IDX_TASK_CREATE
    on eladmin.ACT_RU_TASK (CREATE_TIME_);

create table if not exists eladmin.ACT_RU_TIMER_JOB
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create table if not exists eladmin.ACT_RU_VARIABLE
(
    ID_ varchar(64) not null
        primary key,
    REV_ int null,
...
create index ACT_IDX_VARIABLE_TASK_ID
    on eladmin.ACT_RU_VARIABLE (TASK_ID_);
